...
public class FrontierNavImporter {
	...
	public static ProbeLayout importProbeLayout(Mira mira, String encoded) {
		Map<Site, Probe> probes = Arrays.stream(encoded.split(ImportExportFormat.SITES_DELIMITER))
	...
	public static FrontierNav importFrontierNav(Mira mira, String encoded) {
		ProbeLayout probeLayout = importProbeLayout(mira, encoded);
		return new FrontierNav(mira, probeLayout);
	}

	static void main() throws IOException {
		Mira mira = MiraLoader.loadMira(Path.of("input/sites.tsv"), Path.of("input/network.tsv"), Path.of("input/resources.tsv"));
